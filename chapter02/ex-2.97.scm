...
(define (install-scheme-number-package)
  ...
  (put 'reduce '(scheme-number scheme-number) (lambda (x y)
                                                (let ((g (gcd x y)))
                                                  (list (/ x g) (/ y g)))))
  'done)
...
(define (install-polynomial-package)
  ...
  (define (reduce-terms n d)
    (let ((g (gcd-terms n d)))
      (let ((c (contents (coeff (first-term g))))
            (o1 (max (order (first-term n))
                     (order (first-term d))))
            (o2 (order (first-term g))))
        (let ((int-factor (expt c (+ 1 o1 (- o2)))))
          (let ((temp-term (adjoin-term (make-term 0 int-factor)
                                        (the-empty-termlist))))
            (let ((new-n (car (div-terms (mul-terms n temp-term)
                                         g)))
                  (new-d (car (div-terms (mul-terms d temp-term)
                                         g))))
              (let ((redundant-factor (apply gcd (append (map (lambda (term)
                                                                (contents (coeff term)))
                                                              new-n)
                                                         (map (lambda (term)
                                                                (contents (coeff term)))
                                                              new-d)))))
                (let ((nn (map (lambda (term)
                                 (make-term (order term)
                                            (/ (contents (coeff term))
                                               redundant-factor)))
                               new-n))
  ...
  (put 'reduce '(polynomial polynomial) (lambda (x y)
                                          (let ((result (reduce-poly x y)))
                                            (list (tag (car result))
                                                  (tag (cadr result))))))
  'done)
...
(define (install-rational-package)
  (define (make-rat x y)
    (let ((reduced-items (reduce x y)))
      (let ((new-x (car reduced-items))
            (new-y (cadr reduced-items)))
        (cons new-x new-y))))
